...
let designSystemsSessionId: string | null = null;

async function callDesignSystemsTool(toolName: string, args: Record<string, unknown>): Promise<string> {
  ...
  if (!designSystemsSessionId) {
    ...
    if (sessionHeader) designSystemsSessionId = sessionHeader;
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    switch (name) {
      ...
      case 'get_all_tags': {
        const result = await callDesignSystemsTool(name, (args || {}) as Record<string, unknown>);
