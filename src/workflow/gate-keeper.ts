...
export interface GateKeeperState {
  ...
  designContext: {
    ...
    availableAssets: Array<{ id: string; name: string; type: string }>;
    extractedColors: string[];
    extractedFonts: string[];
  };
  transactionStack: string[]; // For rollback support
}

/**
 * Tools that can be called in each phase
 */
export const TOOL_PHASE_MAP: Record<string, WorkflowPhase> = {
 ...
 */
export function updateStateFromToolResult(
  state: GateKeeperState,
  toolName: string,
  result: unknown
): GateKeeperState {
    ...
    case 'prepare_to_design':
      ...
      if (result && typeof result === 'object') {
        ...
        if (r.isWireframe !== undefined) {
          newState.designContext.isWireframe = r.isWireframe as boolean;
        }
        if (r.wireframeWarning) {
          newState.designContext.wireframeWarning = r.wireframeWarning as string;
        }
      }
      break;
    ...
    case 'get_images':
      ...
      if (result && typeof result === 'object') {
        ...
        if (Array.isArray(r.images)) {
          newState.designContext.availableAssets = r.images.map((img: unknown) => {
    ...
    case 'extract_color_palette':
      ...
      if (result && typeof result === 'object') {
        ...
        if (Array.isArray(r.colors)) {
          newState.designContext.extractedColors = r.colors.map((c: unknown) => {
    ...
    case 'extract_typography_styles':
      if (result && typeof result === 'object') {
        ...
        if (Array.isArray(r.styles)) {
          newState.designContext.extractedFonts = [...new Set(
            r.styles.map((s: unknown) => {
  ...
  if (
    newState.designContext.hasAnalyzedDesign &&
 ...
 */
export function getWorkflowStatus(state: GateKeeperState): string {
  const lines: string[] = [];
